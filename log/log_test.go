...
func TestSlogLogger_Enabled(t *testing.T) {
	...
	if logger.Enabled(lululog.LevelError) != true {
...
func (m *mockLogger) Enabled(level lululog.Level) bool {
	return level >= m.levelToReturn
}

func (m *mockLogger) With(keyvals ...any) lululog.Logger {
	return &mockLogger{
...
var _ lululog.Logger = (*mockLogger)(nil)
...
func TestMultiLogger_FansOutToAll(t *testing.T) {
	a := &mockLogger{}
	b := &mockLogger{}
	ml := MultiLogger{Loggers: []lululog.Logger{a, b}}
...
func TestMultiLogger_FourLevels(t *testing.T) {
	m := &mockLogger{}
	ml := MultiLogger{Loggers: []lululog.Logger{m}}
...
func TestMultiLogger_EnabledAllRequired(t *testing.T) {
	// One logger only accepts Error, one accepts everything.
	// MultiLogger.Enabled should be true only if ALL loggers report enabled.
	a := &mockLogger{levelToReturn: lululog.LevelError} // only Error and above
	b := &mockLogger{levelToReturn: lululog.LevelDebug} // all levels

	ml := MultiLogger{Loggers: []lululog.Logger{a, b}}

	// Debug should be false because logger A doesn't accept it
	if ml.Enabled(lululog.LevelDebug) {
		t.Error("Enabled(Debug) should be false because logger A requires Error")
	}
	// Error should be true because both accept it
	if !ml.Enabled(lululog.LevelError) {
		t.Error("Enabled(Error) should be true because both loggers accept it")
	}
}

func TestMultiLogger_EnabledNoneFalse(t *testing.T) {
	// Both loggers only accept Error, so Debug should not be enabled.
	a := &mockLogger{levelToReturn: lululog.LevelError}
	b := &mockLogger{levelToReturn: lululog.LevelError}
...
func TestMultiLogger_Empty(t *testing.T) {
	ml := MultiLogger{Loggers: []lululog.Logger{}}
	// Should not panic
...
func TestMultiLogger_PanicRecovery(t *testing.T) {
	...
	ml := MultiLogger{Loggers: []lululog.Logger{panicLogger, safeLogger}}
...
func (panickingMock) Enabled(lululog.Level) bool            { return true }
func (p panickingMock) With(...any) lululog.Logger          { return p }

var _ lululog.Logger = panickingMock{}
...
func TestLevelFilter_DiscardsBelowThreshold(t *testing.T) {
	mock := &mockLogger{levelToReturn: lululog.LevelDebug}
	filter := LevelFilter{Logger: mock, Level: lululog.LevelWarn}
...
func TestLevelFilter_PassesAtOrAboveThreshold(t *testing.T) {
	mock := &mockLogger{levelToReturn: lululog.LevelDebug} // accepts all levels
	filter := LevelFilter{Logger: mock, Level: lululog.LevelWarn}
...
func TestLevelFilter_Enabled(t *testing.T) {
	...
	if filter.Enabled(lululog.LevelDebug) {
		t.Error("Enabled(Debug) should be false when threshold is Warn")
	}
	if !filter.Enabled(lululog.LevelWarn) {
		t.Error("Enabled(Warn) should be true")
	}
	if !filter.Enabled(lululog.LevelError) {
		t.Error("Enabled(Error) should be true")
	}
}
...
func TestLevelFilter_With(t *testing.T) {
	...
	if childLF, ok := child.(LevelFilter); ok {
		if childLF.Level != lululog.LevelWarn {
			t.Errorf("child Level = %v, want %v", childLF.Level, lululog.LevelWarn)
		}
	} else {
...
func TestLevelFilter_AllowsAllAtDebugLevel(t *testing.T) {
	mock := &mockLogger{levelToReturn: lululog.LevelDebug} // accepts all levels
	filter := LevelFilter{Logger: mock, Level: lululog.LevelDebug}
