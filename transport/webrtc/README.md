...
本模块提供了一个基于 WebRTC 的 SFU（Selective Forwarding Unit）服务器，实现了 `lulu/transport.Server` 接口，支持音视频流转发、P2P 数据通道、会话管理，可以与 `lulu` 应用框架无缝集成。底层使用 [Pion WebRTC](https://pion.ly/) v4。
...
```bash
go get github.com/kalandramo/lulu-ext/transport/webrtc
```
...
import (
    ...
    webrtc "github.com/kalandramo/lulu-ext/transport/webrtc"
)
...
```powershell
Push-Location "lulu-ext\transport\webrtc"
