...
var ErrServiceInstanceNameEmpty = errors.New("lulu/nacos: ServiceInstance.Name can not be empty")
...
func (r *Registry) Register(_ context.Context, si *lulu.Instance) error {
	if si.Name == "" {
...
func (r *Registry) Deregister(_ context.Context, service *lulu.Instance) error {
...
func (r *Registry) GetService(_ context.Context, serviceName string) ([]*lulu.Instance, error) {
	...
	items := make([]*lulu.Instance, 0, len(res))
	for _, in := range res {
		kind := r.opts.kind
		weight := r.opts.weight
