...
import (
	...
	baseRegistry "github.com/kalandramo/lulu-ext/registry"
)

// Defines the key name of specific fields
// Kratos needs to cooperate with the following fields to run properly on Kubernetes:
// lulu-service-id: define the ID of the service
// lulu-service-app: define the name of the service
// lulu-service-version: define the version of the service
// lulu-service-metadata: define the metadata of the service
// lulu-service-protocols: define the protocols of the service
//
// Example Deployment:
/*
apiVersion: apps/v1
...
template:
  metadata:
    labels:
      app: nginx
      lulu-service-id: "56991810-c77f-4a95-8190-393efa9c1a61"
      lulu-service-app: "nginx"
      lulu-service-version: "v3.5.0"
    annotations:
      lulu-service-protocols: |
        {"80": "http"}
      lulu-service-metadata: |
...
const (
	...
	AnnotationsKeyProtocolMap = "lulu-service-protocols"
)
...
func (s *Registry) Register(ctx context.Context, service *lulu.Instance) error {
	// GetMetadata
...
func (s *Registry) Deregister(ctx context.Context, _ *lulu.Instance) error {
	return s.Register(ctx, &lulu.Instance{
		Metadata: map[string]string{},
	})
}

// GetService return the service instances in memory according to the service name.
func (s *Registry) GetService(_ context.Context, name string) ([]*lulu.Instance, error) {
	...
	ret := make([]*lulu.Instance, 0, len(pods))
...
func (s *Registry) sendLatestInstances(ctx context.Context, name string, announcement chan []*lulu.Instance) {
...
func (s *Registry) Watch(ctx context.Context, name string) (baseRegistry.Watcher, error) {
	stopCh := make(chan struct{}, 1)
	announcement := make(chan []*lulu.Instance, 1)
...
// It reads the latest changes from the `chan []*lulu.Instance`
// And the outside can sense the closure of Iterator through stopCh
type Iterator struct {
	ch     chan []*lulu.Instance
	stopCh chan struct{}
}

// NewIterator is used to initialize Iterator
func NewIterator(channel chan []*lulu.Instance, stopCh chan struct{}) *Iterator {
	return &Iterator{
		ch:     channel,
		stopCh: stopCh,
	}
...
func (iter *Iterator) Next(ctx context.Context) ([]*lulu.Instance, error) {
	select {
...
func getServiceInstanceFromPod(pod *corev1.Pod) (*lulu.Instance, error) {
